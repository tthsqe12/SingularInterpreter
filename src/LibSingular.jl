...
# Sleftv_ below has the same layout as sleftv in Singular, and can be used
# to interpret a pointer comint from c++, e.g.
# > lv::Sleftv # lv is given
# > x = unsafe_load(Ptr{Sleftv_}(lv.cpp_object))

# when used to mirror the c++ types, it seems that this needs
# to be defined before @wrapmodule
mutable struct Sleftv_
    next::Ptr{Sleftv_}
    name::Ptr{Cchar}
    data::Ptr{Cvoid}
...
Sleftv_() = Sleftv_(0, 0, 0, 0, 0, 0, 0, 0)

const Leftv_ = Ptr{Sleftv_}

@wrapmodule(realpath(joinpath(@__DIR__, "..", "local", "lib", "libsingularwrap." * Libdl.dlext)))

@assert sizeof(Sleftv_) == sleftv_sizeof # sleftv_sizeof generated by CxxWrap
