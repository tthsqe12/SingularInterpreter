...
#println("rt_fix_setindex called count_change = "*string(count_change)*", a = "*rt_indenting_print(a,0))
    new_parent = a.parent
...
#println("rt_fix_setindex called count_change = "*string(count_change)*", a = $(a)")
    if count_change > 0
...
#println("success in moving "*rt_indenting_print(b,0)*" from current package to current ring")
                        return
                    else
    ...
    elseif count_change < 0
        ...
        if R.valid && haskey(R.vars, a)
            ...
            if isa(b, SList)
                ...
                delete!(R.vars, a)
#println("success in moving list from current ring to current package")
                return
            else
