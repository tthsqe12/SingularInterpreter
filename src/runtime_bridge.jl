    ...
    if attr !== nothing
        ...
        for (name, val) in attr
            ...
            elseif name == "qringNF"
                ...
                @assert val isa Int # TODO: implement for other kind
                libSingular.sattr_data(a, Ptr{Cvoid}(val))
                libSingular.sattr_name(a, name)
            end
        end
    end
#    id = Sideal(libSingular.ideal(), rt_basering(), false)
#    set_attribute(id, lv.attribute)
#    rtattrib(id)
end
...
function set_attribute(x, from::Sleftv)
    ...
    if attr === nothing && (from.flag != 0 || from.attribute.cpp_object != C_NULL)
        @warn "some attributes have been lost (report as a bug please)"
        return x
    end
    @show from.flag
    ...
    while  at.cpp_object != C_NULL
        ...
        attr[unsafe_string(libSingular.sattr_name(at))] = get_res(T, libSingular.sattr_data(at))
