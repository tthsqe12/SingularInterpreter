...
//                        return std::make_tuple(isnotatuple, Sleftv(res, t));
    ...
    Singular.method("get0", [](void*lv) {
                                sleftv l = *((leftv)lv);
                                std::cout << l.rtyp << ' ' << l.data <<std::endl;
                            });

    Singular.method("get1", [](leftv lv) {
//                                lv.next = &lv; lv.data = (void*)100; lv.rtyp = -1;
                                lv-> next = lv;
                                std::cout << &(*lv) << " -=> " << lv->data << " < " <<lv->rtyp
                                          <<" ( " << lv->next << std::endl;
    });
